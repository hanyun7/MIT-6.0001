...
def contain(word, wrong_letters):
    count = []
    for i in range(len(word)):
        if word[i] in wrong_letters:
            count.append(1)
        else:
            count.append(0)
    if sum(count) >= 1:
...
def show_possible_matches(my_word):
    ...
    for word in wordlist:
        if match_with_gaps(my_word, word) and not contain(word, wrong_letters):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while True:
        ...
        # ENTER THE SAME LETTER WITH WARNING REMAINS
            ...
            show_possible_matches(get_guessed_word(secret_word, letters_guessed), wrong_letters)
            print('\n-------------')
            print('You have', guesses_remaining, 'guesses left.')
            print('Available letters:', get_available_letters(letters_guessed))
            ...
            guesses_remaining = guesses_remaining - 1 if entered_letter in consonants else guesses_remaining - 2
            letters_guessed.append(entered_letter)
            wrong_letters.append(entered_letter)
            print('Oops! That letter is not in my word:', get_guessed_word(secret_word, letters_guessed))
            if guesses_remaining > 0:
                print('-------------')
                print('You have', guesses_remaining, 'guesses left.')
